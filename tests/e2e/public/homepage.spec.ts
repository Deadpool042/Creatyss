...
test("reflects homepage hero updates on the public home page", async ({ page }) => {
  ...
  await expect(page.getByText(/Page d['’]accueil enregistrée avec succès\./i)).toBeVisible();

  await page.goto("/");

  await expect(page.getByRole("heading", { name: heroTitle, exact: true })).toBeVisible();
