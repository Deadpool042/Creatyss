...
test("filters published products by category and availability", async ({
  page
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Sac Camel" })
  ).toHaveCount(0);
  await expect(
    page.getByRole("heading", { name: "Besace Nuit" })
  ).toHaveCount(0);
});

test("combines search with filters and keeps a simple reset path", async ({
  page
}) => {
  await page.goto("/boutique");
