...
async function createCheckoutOrder(
  ...
  await expect(page).toHaveURL(
    /\/boutique\/pochette-sable\?cart_status=added$/
  );
...
async function openAdminOrderFromListByReference(
  page: Page,
  reference: string
): Promise<void> {
  ...
  await Promise.all([
    ...
    createdOrderRow
      .getByRole("link", { name: "Voir le détail", exact: true })
      .click()
  ]);
}
...
test("reflects a shipped order consistently on the public confirmation page", async ({
  ...
  await expect(
    page.getByText("Expédiée", { exact: true }).first()
  ).toBeVisible();
  await expect(
    page
      .getByText(
