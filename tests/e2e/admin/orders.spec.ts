...
test("lets admin cancel a pending order from the detail page", async ({ page }) => {
  ...
  await page.locator('input[name="shippingAddressLine1"]').fill("18 rue du Commerce");
  ...
  await expect(page.getByRole("button", { name: "Marquer en préparation" })).toHaveCount(0);
  ...
  await expect(page.getByText("Le statut de la commande a été mis à jour.")).toBeVisible();
  await expect(page.getByText("Annulée", { exact: true }).first()).toBeVisible();
  await expect(
    page
      .getByText("Aucune autre action n'est disponible pour cette commande.", {
        exact: true,
      })
      .first()
  ).toBeVisible();
...
test("lets admin ship a paid order from the detail page", async ({ page }) => {
  ...
  await page.locator('input[name="shippingAddressLine1"]').fill("22 rue de Rennes");
  ...
  await expect(page.getByRole("heading", { name: "Commande payée" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Commande en préparation" })).toBeVisible();
  ...
  await expect(page.getByText(/Expédiée le /)).toBeVisible();
  await expect(page.getByText("Référence de suivi : COL-123456")).toBeVisible();
  await expect(
