...
function makeCategory(overrides: Partial<CategoryFixture> & { id: string }): CategoryFixture {
  return {
    storeId: STORE_FR_EN,
    name: "Nom FR",
...
describe("resolveLocalizedCategoryCopy", () => {
  it("retourne les champs traduits ACTIVE pour la locale visiteur", async () => {
    ...
    const [result] = await resolveLocalizedCategoryCopy([makeCategory({ id: "category_1" })]);
  ...
  it("retombe sur la valeur canonique si aucune traduction n'existe", async () => {
    ...
    expect(result).toEqual(makeCategory({ id: "category_1" }));
  });
  ...
  it("ne modifie rien si la fonctionnalité multilingue n'est pas active", async () => {
    ...
    const source = makeCategory({ id: "category_1" });
    const [result] = await resolveLocalizedCategoryCopy([source]);
  ...
  it("isole les traductions par boutique (aucune fuite entre stores)", async () => {
    ...
    const [categoryA, categoryB] = await resolveLocalizedCategoryCopy([
      makeCategory({ id: "category_1", storeId: STORE_FR_EN }),
      makeCategory({ id: "category_2", storeId: STORE_OTHER, name: "Nom autre boutique" }),
    ]);

    expect(categoryA?.name).toBe("Name EN");
    expect(categoryB?.name).toBe("Nom autre boutique");
