...
export function AdminSplitView({
  ...
  const isDetailActive =
    pathname !== listRootPath && pathname.startsWith(listRootPath + "/");
  const {
    desktopListWidth,
  ...
  const desktopListStyle = !desktopListCollapsed
    ? { width: desktopResizable ? `${desktopListWidth}px` : `${defaultDesktopListWidth}px` }
    : undefined;
    ...
    <div className="relative flex min-h-0 flex-1 overflow-hidden">
      ...
      <div
        className={cn(
          ...
          isDetailActive ? "-translate-x-full pointer-events-none" : "translate-x-0",
      ...
      {desktopResizable ? (
        ...
        >
          {!showDesktopRail ? (
            ...
            <span className="my-2.5 mx-auto w-px flex-1 rounded-full bg-surface-border-subtle" />
          )}
        </div>
      ) : null}
      ...
      <div
        className={cn(
          "absolute inset-0 flex flex-col overflow-hidden",
          "transition-transform duration-(--motion-duration-panel) ease-(--motion-ease-spring)",
          isDetailActive ? "translate-x-0" : "translate-x-full pointer-events-none",
          "lg:pointer-events-auto lg:relative lg:inset-auto lg:translate-x-0",
