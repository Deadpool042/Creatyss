/**
 * Dictionnaire fr du copy homepage (Horizon 4 — lot 3, « convention copy
 * unifiée »).
 *
 * Contenu de la locale par défaut, résolu via
 * `entities/languages/resolve-locale-content.ts` par
 * `features/homepage/config/homepage-copy.config.ts` (le contrat — forme et
 * clés consommées par les composants). Issu de l'extraction R1
 * (2026-06-12) ; mêmes valeurs, déplacées ici sans changement.
 */

export const HOMEPAGE_COPY_FR = {
  hero: {
    fallbackText:
