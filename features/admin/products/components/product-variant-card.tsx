    ...
    <article className="admin-variant-card grid gap-6 rounded-xl border border-border/70 bg-card p-5 text-card-foreground shadow-sm">
      <div className="grid gap-3">
        <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
          {isSimpleProduct ? "Déclinaison existante" : productPresentation.itemKicker}
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <Badge variant="outline">{getProductStatusLabel(variant.status)}</Badge>
        </div>
        <h3>{variant.name}</h3>
        ...
        <fieldset className={fieldsetClassName}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <AdminFormField htmlFor={skuId} label="SKU">
              <Input defaultValue={variant.sku} id={skuId} name="sku" required type="text" />
            </AdminFormField>
        ...
        <fieldset className={fieldsetClassName}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <AdminFormField htmlFor={nameId} label="Nom de la déclinaison">
              <Input defaultValue={variant.name} id={nameId} name="name" required type="text" />
            </AdminFormField>
          ...
          <label className="flex items-start gap-3 text-sm leading-6 text-foreground">
            ...
            />
            <span>
              {isSimpleProduct ? "Déclinaison par défaut" : "Définir comme déclinaison par défaut"}
            </span>
          </label>
        </fieldset>
      ...
      <div className="grid gap-2">
        <p className="text-sm font-bold uppercase tracking-[0.08em] text-brand">
          {isSimpleProduct ? "Images existantes" : productPresentation.imagesEyebrow}
        </p>
        <h3>
          {isSimpleProduct
        ...
        <p className="card-copy text-sm leading-6 text-muted-foreground">
          Commencez par l&apos;image principale de cette déclinaison. Les autres réglages
          d&apos;images restent disponibles plus bas si nécessaire.
        </p>
      </div>
      ...
      <div className="grid gap-2">
        ...
        <p className="text-sm leading-6 text-muted-foreground">
          Les autres images associées et leurs réglages restent disponibles ici si vous devez
          intervenir plus finement.
        </p>
      </div>

      {hasMediaAssets ? (
        <form action={createProductImageAction} className="space-y-4">
          <input name="productId" type="hidden" value={productId} />
          <input name="variantId" type="hidden" value={variant.id} />
          <input name="imageScope" type="hidden" value="variant" />

          <AdminFormField htmlFor={mediaAssetId} label="Média existant">
            <select
              className={nativeSelectClassName}
              defaultValue=""
              id={mediaAssetId}
          ...
          <AdminFormField htmlFor={altTextId} label="Texte alternatif">
            <Input id={altTextId} name="altText" type="text" />
          </AdminFormField>

          <AdminFormField htmlFor={sortOrderId} label="Ordre">
            <Input defaultValue="0" id={sortOrderId} name="sortOrder" type="number" />
          </AdminFormField>

          <label className="flex items-start gap-3 text-sm leading-6 text-foreground">
            <input className={checkboxInputClassName} name="isPrimary" type="checkbox" value="on" />
