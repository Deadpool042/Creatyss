...
export function ProductImagesTab({
  ...
  const [localUploadFormOpen, setLocalUploadFormOpen] = useState(false);

  const effectiveLibraryOpen = attachLibraryOpen ?? localLibraryOpen;
  const effectiveUploadFormOpen = uploadFormOpen ?? localUploadFormOpen;

  function setEffectiveLibraryOpen(open: boolean): void {
    if (onAttachLibraryOpenChange) {
      onAttachLibraryOpenChange(open);
    } else {
      setLocalLibraryOpen(open);
    }
  }

  function setEffectiveUploadFormOpen(open: boolean): void {
    if (onUploadFormOpenChange) {
      onUploadFormOpenChange(open);
    } else {
      setLocalUploadFormOpen(open);
    }
  }
  ...
  async function handleDelete(
    ...
    const result = await deleteImageAction({ productId, assetId });
    setMessageState(result);
    return result;
  }
  ...
  async function handleUpdateAltText(
    ...
    const result = await updateAltTextAction({ productId, assetId, altText });
    setMessageState(result);
    return result;
  }
  ...
  async function handleReorder(
    ...
    const result = await reorderImageAction({ productId, assetId, direction });
    setMessageState(result);
    return result;
  }
  ...
  async function handleAttach(
    ...
    const result = await attachImagesAction({ productId, assetIds });
    setMessageState(result);
    return result;
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="min-h-0 flex-1 overflow-y-auto pt-28.25 pb-[calc(3.5rem+env(safe-area-inset-bottom))] sm:pt-29.25 md:pt-30.25 [@media(max-height:480px)]:pt-25.25 [@media(max-height:480px)]:pb-[calc(3rem+env(safe-area-inset-bottom))] lg:pt-17.25 lg:pb-0">
        <div className="w-full space-y-5 px-4 pt-4 pb-3 md:space-y-8 md:px-6 md:pt-6 md:pb-6 lg:mx-auto lg:max-w-6xl lg:px-4 xl:px-0 [@media(max-height:480px)]:space-y-4 [@media(max-height:480px)]:pt-3 [@media(max-height:480px)]:pb-2.5">
          <AdminFormMessage
