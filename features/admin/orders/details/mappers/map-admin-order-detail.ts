...
import type {
  ...
  AdminOrderPayment,
  AdminOrderShipment,
} from "@/features/admin/orders/details/types/admin-order-detail.types";
...
export function mapAdminOrderDetail(input: {
  ...
  const shippingAddress = input.order.addresses.find((address) => address.type === "SHIPPING") ?? null;
  const billingAddress = input.order.addresses.find((address) => address.type === "BILLING") ?? null;
