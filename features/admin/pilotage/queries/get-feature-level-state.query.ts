 ...
 *
 * Généralise `getLocalizationFeatureState`/`meetsLocalizationLevel`
 * (`features/localization/queries/get-localization-feature-state.query.ts`,
 * premier câblage de la gradation) à n'importe quel `FeatureFlag.code` —
 * notamment les entrées `core` `catalog.products.inventory` et
 * `catalog.products.media` (cf.
 * `docs/lots/2026-06-13-inventory-media-levels-cadrage.md`, sous-lot 0).
 *
 ...
 */
export async function getFeatureLevelState(code: string): Promise<FeatureLevelState> {
 ...
 */
export async function meetsFeatureLevel(code: string, requiredLevel: string): Promise<boolean> {
  const state = await getFeatureLevelState(code);
