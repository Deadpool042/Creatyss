...
type GuestCartItemReferenceRow = { id: string; product_variant_id: string; quantity: number };
...
function mapGuestCartItemReference(row: GuestCartItemReferenceRow): GuestCartItemReference {
  return { id: row.id, variantId: row.product_variant_id, quantity: row.quantity };
}
...
function buildGuestCartFromLineRows(rows: readonly GuestCartLineRow[]): GuestCart | null {
  ...
  return { id: firstRow.cart_id, itemCount, subtotal: centsToMoneyString(subtotalCents), lines };
}

export async function findGuestCartIdByToken(token: string): Promise<string | null> {
  const row = await queryFirst<GuestCartIdRow>(
    `select id::text as id from carts where token = $1 limit 1`,
    [token]
  );
  return row?.id ?? null;
}
...
export async function createGuestCart(token: string): Promise<string> {
  const row = await queryFirst<GuestCartIdRow>(
    `insert into carts (token) values ($1) returning id::text as id`,
    [token]
  );
  if (!row) throw new Error("Guest cart creation failed.");
  return row.id;
}
...
export async function readGuestCartByToken(token: string): Promise<GuestCart | null> {
  const rows = await queryRows<GuestCartLineRow>(
    `select c.id::text as cart_id, ci.id::text as id,
      ci.product_variant_id::text as product_variant_id, ci.quantity,
      p.id::text as product_id, p.slug as product_slug, p.name as product_name,
      p.status as product_status, pv.name as variant_name, pv.color_name, pv.color_hex,
      pv.sku, pv.price::text as unit_price, pv.stock_quantity, pv.status as variant_status,
...
export async function readGuestCheckoutDetailsByCartId(
  ...
  const row = await queryFirst<GuestCartCheckoutDetailsRow>(
    `select id::text as id, cart_id::text as cart_id, customer_email,
      ...
      shipping_address_line_1, shipping_address_line_2, shipping_postal_code,
      shipping_city, shipping_country_code, billing_same_as_shipping,
...
export async function upsertGuestCheckoutDetails(input: {
  ...
  const row = await queryFirst<GuestCartCheckoutDetailsRow>(
    `insert into cart_checkout_details (
      cart_id, customer_email, customer_first_name, customer_last_name, customer_phone,
      shipping_address_line_1, shipping_address_line_2, shipping_postal_code, shipping_city,
    ...
    returning id::text as id, cart_id::text as cart_id, customer_email,
      customer_first_name, customer_last_name, customer_phone,
      shipping_address_line_1, shipping_address_line_2, shipping_postal_code,
      shipping_city, shipping_country_code, billing_same_as_shipping,
      billing_first_name, billing_last_name, billing_phone,
  ...
  if (!row) throw new Error("Guest checkout details save failed.");
