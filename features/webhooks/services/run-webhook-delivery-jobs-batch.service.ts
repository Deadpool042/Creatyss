...
import { executeWebhookDeliveryJob } from "./execute-webhook-delivery-job.service";
...
export type RunWebhookDeliveryJobsBatchStats = {
  recovered: number;
  requeued: number;
  selected: number;
  succeeded: number;
...
export async function runWebhookDeliveryJobsBatch(
  batchSize: number = 50
): Promise<RunWebhookDeliveryJobsBatchStats> {
  const now = new Date();
  ...
  for (const job of dueJobs) {
    ...
    try {
      await executeWebhookDeliveryJob({ jobId: job.id, storeId: job.storeId });
      succeeded++;
    } catch {
      failed++;
    }
