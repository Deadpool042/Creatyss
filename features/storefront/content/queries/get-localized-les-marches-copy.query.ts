...
import {
  ...
  type LesMarchesPageCopyDictionary,
...
/**
 * Résout le dictionnaire page Les marchés en y appliquant les `LocalizedValue`
 * ACTIVE de la locale visiteur pour les champs de
 * `LES_MARCHES_PAGE_COPY_FIELDS`.
 *
 * Retourne `contentPagesCopyConfig.lesMarches` inchangé si :
 ...
 */
export async function getLocalizedLesMarchesCopy(): Promise<LesMarchesPageCopyDictionary> {
  const allowed = await meetsFeatureLevel(LOCALIZATION_FEATURE_CODE, "multilingual");

  if (!allowed) {
    return contentPagesCopyConfig.lesMarches;
  }
  ...
  const values = await db.localizedValue.findMany({
    where: {
      ...
      fieldName: { in: LES_MARCHES_PAGE_COPY_FIELDS.map((field) => field.fieldName) },
      status: "ACTIVE",
      archivedAt: null,
  ...
  return withLesMarchesPageCopyOverrides(contentPagesCopyConfig.lesMarches, overrides);
}
