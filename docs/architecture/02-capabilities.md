...
- d’un comportement neutre et cohérent quand une capability est désactivée
...
La configuration précise **comment** la capability fonctionne.
...
Un feature flag est un mécanisme de rollout ou d’expérimentation.
Il ne remplace pas une capability.

Exemple :

- capability active
- feature flag temporairement à `false` sur une nouvelle implémentation

## Règle

- **capability** = possibilité métier/plateforme offerte à la boutique
- **configuration** = paramètres de cette capability
- **feature flag** = pilotage technique progressif
...
Une capability off doit aussi désactiver l’exposition fonctionnelle côté UI/admin.
