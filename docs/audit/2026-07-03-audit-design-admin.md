...
4. **Vues détail à cartes multiples** (`orders/[id]`, potentiellement `products/[slug]/*`) — **livré le 2026-07-04** pour `orders/[id]` : regroupement en 3 onglets (`OrderDetailTabs`, shadcn `Tabs`) — Résumé + Actions restent toujours visibles, le reste (client/livraison, traitement, historique) est en onglets. `products/[slug]/*` non traité dans ce lot, à évaluer séparément si le besoin se confirme.
5. **Pages listes tabulaires denses** (`catalog/pricing`, `content/blog`, `settings/team`, `settings/api-clients`) — les `grid-cols-3` non responsive ont été corrigées dès le lot 1. **Piste `Table` native écartée après vérification (2026-07-04)** : les listes de `team`, `api-clients` et `blog` utilisent déjà un seul niveau de cadre (`divide-y`) avec masquage progressif des colonnes secondaires (`hidden lg:block`) — un motif responsive fonctionnel. Convertir en `<table>` HTML serait une régression pour le mobile (une table native ne divulgue pas progressivement ses colonnes aussi bien qu'une ligne flex). Aucune conversion effectuée.
