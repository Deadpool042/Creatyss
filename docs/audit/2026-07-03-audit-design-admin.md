...
3. **Formulaires de réglages** (`commerce/shipping/settings` et familles `settings/*`) — déjà plus proches d'un bon niveau d'aération (contre-exemple positif) ; vérifier que le reste de la famille `settings/*` suit ce même motif plat plutôt que le motif carte-dans-carte, avant de les considérer comme référence à généraliser.
