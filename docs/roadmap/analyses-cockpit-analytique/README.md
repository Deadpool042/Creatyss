...
5. **Cockpit consolidé** — une fois 1 à 4 avancés, revoir `/admin/insights/analytics` pour intégrer les nouvelles sources sans reproduire un hub à cartes creux (leçon du lot 14 : ne pas ajouter de navigation qui ne mène qu'à du vide). **Cadré, non implémenté (2026-07-06)** : cf. `lot-5-cockpit-consolide-cadrage.md` — la matière réellement consolidable est plus modeste que prévu (recherche : donnée déjà écrite, juste pas lue ; conversion et attribution : rien de neuf à afficher dans ce cockpit). Trois questions produit à trancher avant code.
