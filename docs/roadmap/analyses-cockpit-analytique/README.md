...
4. **Attribution marketing** — activer `attribution.md` (source/canal/campagne) si le besoin de mesure est confirmé. **Cadré, non implémenté (2026-07-06)** : cf. `lot-4-attribution-cadrage.md` — fork architectural identifié (capture UTM + mémorisation jusqu'à conversion, premier cas du repo qui casserait le principe d'anonymat sans identifiant du tracking interne), recommandation de laisser porter la lecture par les providers analytics externes déjà en place (lot 1) plutôt que construire une mémorisation interne. La dépendance aux lots 2/3 énoncée initialement s'avère inexacte : ils ne produisent pas d'événement individuellement attribuable. Décision d'implémentation non tranchée.
