...
- **Recherche** (termes recherchés, résultats, conversion recherche→achat) : aucun tracking, aucune query, aucun modèle de données identifié. Le domaine `search` existe (index FTS Postgres, `prisma/satellites/search.prisma`) mais ne journalise pas les requêtes utilisateur.
