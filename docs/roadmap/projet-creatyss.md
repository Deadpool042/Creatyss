...
**Mise à jour 2026-06-17 (smoke E2E) :** le smoke `tests/e2e/public/commerce-smoke.spec.ts` passe localement après réalignement sur le contrat actuel des références de commande. Parcours couvert : produit test vendable → ajout panier → panier visible → checkout invité (informations enregistrées) → livraison sélectionnée → paiement `Virement bancaire` → création commande → confirmation publique (référence à préfixe boutique configurable, forme `PREFIX-XXXXXXXXXX`, email client, paiement en attente) → panier vidé → connexion admin → commande visible dans la vue d'ensemble → détail admin (email client, statut paiement, méthode `Virement bancaire`).
...
- [x] Parcours testé de bout en bout — smoke E2E minimal vert localement, revalidé le 2026-06-17 avec le format de référence configurable
...
**Mise à jour 2026-06-29 (déploiement complet via script) :** `scripts/deploy.sh` exécuté sur VPS OVH — déploiement complet validé de bout en bout. Commit déployé : `062b2d4c`. HTTPS : HTTP/2 200. Healthcheck interne : `{"app":"ok","database":"ok","uploads":"ready"}`. Image rollback : `creatyss:prod-20260629-084301`. Sauvegardes produites : DB + uploads + documents. Conteneurs : `creatyss-app` up · `creatyss-db` healthy · `creatyss-caddy` up. Points non bloquants : image placeholder uploads absente (log app), backups générés `root:root` (à surveiller). Production finale `creatyss.com` non encore basculée.

**Mise à jour 2026-06-29 (recette commerce staging) :** parcours achat complet validé manuellement sur `https://staging.creatyss.lpwebstudio.fr`. Commit : `47aefc40`. Feature flags 32/32 matérialisés, prix canoniques vérifiés en DB (4 produits, valeurs en centimes : mini-sac-clarte 9 900 c / 99,00 EUR, bandouliere-rivoli 12 900 c / 129,00 EUR, pochette-nocturne 5 900 c / 59,00 EUR, cabas-atelier 15 900 c / 159,00 EUR). Parcours virement de bout en bout validé — deux commandes créées en DB (`CRY-XPDCA6TTXG` 105,90 EUR, `CRY-6DCG97F94E` 135,90 EUR). Backup pré-déploiement : `20260629-205224`. Recette Stripe et bascule `creatyss.com` hors périmètre. Détail : `docs/exploitation/06-recette-commerce-complete.md`.

**Mise à jour 2026-06-30 (recette admin commerce staging) :** administration des commandes validée manuellement sur staging. Validé : consultation liste et détail commande, capture virement puis traitement complet de `CRY-XPDCA6TTXG` jusqu'à `COMPLETED`, préparation commande, expédition avec persistance carrier/tracking (`CRY-DL5FBSCRD2` → carrier `Colissimo`, tracking `colissimo-123`), livraison shipment (`CRY-XPDCA6TTXG` → `DELIVERED`, `deliveredAt` en DB), annulation commande PENDING (`CRY-6DCG97F94E` → `CANCELLED`). Non validé : email `order_shipped` réel, retours/remboursements, documents/Factur-X. Détail : `docs/exploitation/06-recette-commerce-complete.md`.

**Mise à jour 2026-06-30 (Stripe test et Brevo transactionnel validés) :** Stripe test configuré et validé sur staging — webhook `https://staging.creatyss.lpwebstudio.fr/api/stripe/webhook` fonctionnel, événement `checkout.session.completed` accepté, commande créée en DB avec statut `CONFIRMED` (`CRY-GGJ7BALGFB`). Diagnostic initial Stripe : fichier `.env` contenait deux valeurs `STRIPE_WEBHOOK_SECRET` ; Docker utilisait la mauvaise. Correction : une seule valeur cohérente avec destination webhook active. Brevo transactionnel validé : API REST configurée (clé `xkeysib…`), IP VPS OVH `51.38.37.67` autorisée, email envoyé/reçu. Diagnostic initial Brevo : clé SMTP (`xsmtpsib`) utilisée au lieu de clé API REST ; pare-feu Brevo bloquait IP VPS. Corrections : bascule vers clé API REST et autorisation IP. Production finale `creatyss.com` non encore basculée. Détail : `docs/exploitation/06-recette-commerce-complete.md`.
...
- [x] Recette commerce staging validée — parcours virement complet, 2 commandes en DB, prix canoniques vérifiés (2026-06-29) ; Stripe et bascule `creatyss.com` hors périmètre
...
- `localization` comme première capacité graduée de référence : valide le pattern complet (toggle + niveaux + routing + traduction du contenu métier, distincte du copy UI). Lots 1 à 3 faits (2026-06-13) : guard gradué branché, niveau `managed` câblé (locale par défaut gérée + admin minimal `/admin/settings/localization`), convention copy unifiée (`entities/languages/resolve-locale-content.ts`), pilote `homepage` migré sans changement de composant. Lot 4 (`multilingual`, sous-lots 1-5) fait (2026-06-13) : seconde locale gérée, lecture `LocalizedValue` + fallback, sélecteur de langue storefront sans routing, admin de traduction et câblage lecture multilingue du pilote homepage. Lot 5 (`localized-routing`, L3) implémenté le 2026-06-16 (`proxy.ts`, `app/layout.tsx`, `app/sitemap.ts`, redirection locale). Généralisation `LocalizedValue` poursuivie ensuite sur `product-page-copy`, `boutique-page-copy`, pages de contenu (`contact`, `a-propos`, `les-marches`) puis blog (`title`, `excerpt`, `content`) côté admin + storefront le 2026-07-05, puis champs métier produit (`name`, `shortDescription`, `description`) côté admin + storefront (fiche produit, listing, favoris) le 2026-07-22, puis SEO produit et produits associés (`RelatedProduct`) le 2026-07-23, puis catégories (`name`, `shortDescription`, `description`) et emails transactionnels de commande (`order_created`, `payment_succeeded`, `order_shipped`, via `Order.localeCode`) le 2026-07-23. Chantier mis en pause le 2026-07-23 (décision utilisateur) — reste hors périmètre à la reprise : pages statiques additionnelles, automation marketing, newsletter (cf. `docs/roadmap/h4-plateforme-automatisation/lot-multilangue-generalise.md` pour repères détaillés).
