...
**Mise à jour 2026-07-19** : le formulaire storefront de **vérification d'éligibilité** (sans création de `ReturnRequest`, distinct de la demande) est livré sur `/compte` — PR #16, commit `18269fed`. Détail : `docs/roadmap/h2-commerce-fiable/README.md` (section « Mise à jour 2026-07-19 »).

**Mise à jour 2026-07-23** : le formulaire storefront de **soumission de demande** (création réelle d'un `ReturnRequest`) est livré, cadrage préalable `architect-review`. Parcours en 2 étapes sur `/compte` : après un résultat d'éligibilité `ELIGIBLE`/`MANUAL_REVIEW` (étape 1, déjà livrée), `ReturnSubmissionForm` s'affiche pour la sélection lignes/quantités/motif (étape 2). Décision produit tranchée : `MANUAL_REVIEW` autorise aussi la création (statut `REQUESTED`, traité ensuite par l'admin dans sa file existante via `UNDER_REVIEW`) — seul `INELIGIBLE` bloque. Sécurité : la Server Action de soumission (`submit-storefront-return-request-action.ts`) ré-identifie systématiquement la commande (référence + email) et ré-évalue l'éligibilité côté serveur avec les lignes réellement choisies — aucune confiance dans un `orderId` ou un résultat d'éligibilité transmis par le client. `createReturnRequest` (service admin) étendu de façon additive (`reasonCode`/`notes` optionnels), réutilisé tel quel — aucune modification du workflow admin existant. Contrat public minimal identique à l'éligibilité (`{ available: false }` indiscernable entre tous les cas d'échec, `{ available: true, status: "SUBMITTED" }` sans détail au succès).
