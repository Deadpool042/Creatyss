...
Terminé. `lot-decision-ia-admin.md` validé (2026-07-02). Les deux options (B puis A) sont livrées et fusionnées dans `main` :

- **Option B (resync hub/sidebar)** — livré (`feature/admin-nav-settings-sync`). Hub `/admin/settings` dérivé de `admin-navigation.data.ts` via `buildAdminSettingsHubItems`, filtré par capabilities réelles (avant : aucune vérification d'accès sur le hub). 5 labels "Vue d'ensemble" différenciés par domaine.
- **Option A, pilote domaine `customers`** — livré (`feature/admin-nav-settings-sync`). `settings/customers` (stub `AdminComingSoon`) fusionné dans `commerce/customers/settings`, avec redirect conservé et nav locale à 2 entrées (`CustomerRouteNav`). Item `customers-settings` supprimé de `admin-navigation.data.ts` (pas seulement masqué, pour éviter un doublon dans le menu mobile).
- **Option A, 5 domaines restants (catalog, orders, media, payments, shipping)** — livré en un seul commit (`f9f7baea`, déjà dans `main`), suivant le pattern ci-dessous : redirect conservé sur chaque `settings/<domaine>`, route `<groupe-domaine>/<domaine>/settings` créée, nav locale à 2 entrées par domaine (`payment-route-nav.tsx`, `shipping-route-nav.tsx`, `media-route-nav.tsx`, `order-route-nav.tsx`, `product-route-nav.tsx`), item `<domaine>-settings` retiré de `admin-navigation.data.ts` pour les domaines sans duplication de label restante.

Les 6 domaines identifiés par l'audit sont désormais fusionnés. Vérification responsive live non exécutée sur l'ensemble des sous-lots (pas de canal navigateur disponible en session) — à faire dès qu'un canal navigateur est disponible.
...
## Pattern établi par le pilote `customers` (répliqué ensuite pour catalog, orders, media, payments, shipping)

Pour chaque paire dupliquée :
...
Les 5 domaines catalog/orders/media/payments/shipping avaient des formulaires réels (41 à 155 lignes, cf. audit section 3.4) contrairement au stub `customers` — migrés en un seul commit (`f9f7baea`) plutôt qu'en sous-lots séquentiels distincts comme initialement prévu ci-dessus.
...
- Navigation manuelle recettée sans 404 sur l'ensemble des groupes touchés — **non exécutée** (pas de canal navigateur disponible en session), à faire dès que possible.
- Aucune capability ou feature flag perdu par rapport à l'état avant lot — vérifié par lecture de code (redirects + capabilities conservées sur chaque route migrée).
